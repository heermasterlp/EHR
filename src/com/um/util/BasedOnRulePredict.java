...
public class BasedOnRulePredict {
	...
	public static List<String> predictBasedOnRules(String description){
		...
		// 6. 胸肋痛----延胡索，，，牛蒡子OR 木蝴蝶
		if (description.contains("胸肋痛（轻）") ||description.contains("胸肋痛（中）") ||description.contains("胸肋痛（重）")) {
			medicineSet.add("延胡索");
		...
		// 9. 睡眠差 －－  酸枣仁 ＋ 磁石
		if (description.contains("失眠") || description.contains("嗜睡") ) {
			medicineSet.add("酸枣仁");
			medicineSet.add("磁石");
		}
		// 10. 红血痰 －－－ 紫珠草 ＋ 三七
		...
		if (description.contains("口渴多饮")) {
			medicineSet.add("有瓜石斛");
			medicineSet.add("天花粉");
		}
		
		// 12. 腹泻 泄泻 ----- 石榴皮，五味子，补骨脂， 淮山药 五选一
		if (description.contains("腹泻")) {
			
			Random random = new Random();
			String[] medicines = {"石榴皮","五味子","补骨脂","山药","葶苈子"};
			int low = 0;
			int high = 5;
			int result = random.nextInt(high - low) + low;
			medicineSet.add(medicines[result]);
			...
			while( result == result1){
				result1= random.nextInt(high - low) + low;
			}
			medicineSet.add(medicines[result1]);
		}
